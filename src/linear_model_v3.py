#!/usr/bin/env python
"""
Autoencoding Meta-Embedding with linear model and dimension restraints
"""
...
def train_embedding(source_list, output_path, learning_rate, batch_size, epoch):
    ...
    with tf.name_scope('Encoder1'):
        w_E1 = tf.Variable(tf.random_normal(shape=[150, 300], stddev=0.01), name='w_E1')
        b_E1 = tf.Variable(tf.zeros([150, 1]), name='b_E1')
        tf.summary.histogram('w_E1', w_E1)
        tf.summary.histogram('b_E1', b_E1)
    with tf.name_scope('Encoder2'):
        w_E2 = tf.Variable(tf.random_normal(shape=[150, 300], stddev=0.01), name='w_E2')
        b_E2 = tf.Variable(tf.zeros([150, 1]), name='b_E2')
        tf.summary.histogram('w_E2', w_E2)
        tf.summary.histogram('b_E2', b_E2)
    with tf.name_scope('Decoder1'):
        w_D1 = tf.Variable(tf.random_normal(shape=[300, 150], stddev=0.01), name='w_D1')
    ...
    with tf.name_scope('Decoder2'):
        w_D2 = tf.Variable(tf.random_normal(shape=[300, 150], stddev=0.01), name='w_D2')
    ...
    for word in inter_words:
        meta_embedding[word] = np.concatenate([(np.dot(w_E1, cbow_dict[word].reshape((300, 1))) + b_E1).reshape((150)), (np.dot(w_E2, glove_dict[word].reshape((300, 1))) + b_E2).reshape((150))])
